...
//        setBieuDoDoanhThu(jpnThongKe);
    ...
    private void loadDataThongKe(List<CTSanPhamHD> list) {
        ...
        int count = 1;//đánh số thứ tự của các hàng được thêm vào bảng.
    ...
    private void loadDataDoanhThu(List<Integer> list) {
        ...
        for (int col = 2; col <= 4; col++) {//Vòng lặp này duyệt qua các cột từ 2 đến 4 (bao gồm) và thiết lập một trình hiển thị tùy chỉnh cho mỗi cột
            columnModel.getColumn(col).setCellRenderer(new View.CurrencyRenderer());//có trách nhiệm hiển thị các ô bằng cách sử dụng định dạng tiền tệ.
        }
...
//    }
    private int sumHoaDon(List<HoaDon> list) {
        int count = 0;//nó khởi tạo một biến count với giá trị ban đầu là 0
        for (HoaDon bill : list) {// sau đó lặp qua từng đối tượng HoaDon trong danh sách sử dụng vòng lặp for-each
            count++;//Đối với mỗi đối tượng HoaDon gặp phải, nó tăng giá trị của count lên 1
        }
        return count;// Cuối cùng, nó trả về giá trị cuối cùng của count
    }

    private Date getNgayHienTai() {
        LocalDate localDate = LocalDate.now();//sử dụng lớp LocalDate để lấy ngày hiện tại từ hệ thống
        return Date.valueOf(localDate);//chuyển đổi LocalDate thành đối tượng Date bằng cách sử dụng phương thức valueOf() của lớp Date
    }

    private String getMonth() {
        LocalDate currentDate = LocalDate.now();//sử dụng lớp LocalDate để lấy ngày hiện tại từ hệ thống
        int month = currentDate.getMonthValue();//lấy giá trị của tháng từ đối tượng LocalDate bằng cách sử dụng phương thức getMonthValue()
        String monthString = String.format("%02d", month);//định dạng giá trị của tháng thành một chuỗi hai chữ số bằng cách sử dụng String.format() với mẫu "%02d"
        return monthString;//trả về chuỗi monthString
    }

    private String getYear() {
        LocalDate currentDate = LocalDate.now();//sử dụng lớp LocalDate để lấy ngày hiện tại từ hệ thống
        int year = currentDate.getYear();//lấy giá trị của năm từ đối tượng LocalDate bằng cách sử dụng phương thức getYear()
        String yearString = String.valueOf(year);//chuyển đổi giá trị của năm thành một chuỗi bằng cách sử dụng phương thức String.valueOf()
        return yearString;//trả về chuỗi yearString
    }
...
//        return 0 + ".VND";
//    }
    private String formatCurrency(double amount) {//chuyển đổi một số tiền (kiểu double) thành định dạng tiền tệ dưới dạng chuỗi String
        DecimalFormat decimalFormat = new DecimalFormat("#,##0");//tạo một đối tượng DecimalFormat với mẫu "#,##0" để định dạng số theo định dạng có dấu phẩy phân tách hàng nghìn và không có phần thập phân
        String formattedAmount = decimalFormat.format(amount);//sử dụng phương thức format() của DecimalFormat để định dạng số tiền và lưu kết quả vào formattedAmount, là chuỗi đã được định dạng
        return formattedAmount + " VNĐ";//formattedAmount kết hợp với đơn vị tiền tệ "VNĐ"
    }

    private String sum(List<Double> list) {//tính tổng các giá trị trong một danh sách các số thực
        double totalSum = 0.0;// totalSum với giá trị ban đầu là 0.0

        for (Double sum : list) {
            if (sum != null) {//Trong mỗi lần lặp, nếu số khác null, nó cộng giá trị của số đó vào totalSum
                totalSum += sum;
            }
        }

        return formatCurrency(totalSum);//gọi formatCurrency(totalSum) để định dạng totalSum thành chuỗi định dạng tiền tệ và trả về kết quả
    }
