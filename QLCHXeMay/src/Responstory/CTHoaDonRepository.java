...
public class CTHoaDonRepository {
    ...
    public List<Double> sumDate(Date date, int trangThai) throws SQLException {
        try {
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");//định dạng đối tượng Date thành một chuỗi ngày tháng năm
            ...
            ps.setString(1, dateString);//phương thức thiết lập các giá trị tham số trong câu lệnh SQL bằng cách sử dụng phương thức setString() và setInt()
            ps.setInt(2, trangThai);
            ResultSet rs = ps.executeQuery();//phương thức thực thi câu lệnh SQL và nhận kết quả trả về từ bảng hoadon
            while (rs.next()) {//phương thức lặp qua các hàng 
                double sum = rs.getDouble(1);//lấy giá trị của cột đầu tiên (SUM(hoadon.sotienphaitra)) từ hàng hiện tại và thêm vào danh sách list.
                list.add(sum);
            }
    ...
    public List<CTSanPhamHD> getListThongKe(String ngayBatDau, String ngayKetThuc, int trangThai) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("d 'thg' M, yyyy");//định dạng các chuỗi ngày bắt đầu và ngày kết thúc theo mẫu "d 'thg' M, yyyy"
        Date startDate = null;
        Date endDate = null;
        try {//chuyển đổi chuỗi ngày bắt đầu và chuỗi ngày kết thúc thành đối tượng Date bằng cách sử dụng đối tượng dateFormat và phương thức parse()
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {//phương thức lặp qua các hàng của ResultSet bằng cách sử dụng vòng lặp while
                CTSanPhamHD ctsp = new CTSanPhamHD();//tạo một đối tượng CTSanPhamHD và đặt giá trị mã sản phẩm (masp) và số lượng (tong) từ hàng hiện tại vào đối tượng ctsp, sau đó thêm ctsp vào danh sách list.
